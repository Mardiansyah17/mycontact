...
Route::get('/', function (Request $request) {
    if (request('search')) {
        $contacts =   Contact::with('category')->where('category_id', '=', null)->orWhere('category_id', '!=', 2)->latest()->where('user_id', Auth()->id())->where('name', 'like', '%' . request('search') . '%')->get()->sortBy('name', SORT_NATURAL | SORT_FLAG_CASE);
    } else {
        $contacts = Contact::with(['user', 'category'])->where('category_id',  null)
